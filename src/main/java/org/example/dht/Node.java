...
public class Node {
	...
	public Node (InetSocketAddress address, Map config) {
		this(address);
		try {
			String data2dest = (String) config.get("data.dest");
			StringMetadataCache.cleanUp();
			tempDirForStore = Path.of(data2dest);
			tempDirForStore = tempDirForStore.resolve(localAddress.getAddress().getHostAddress() + "_" + localAddress.getPort());
	...
	public boolean join (InetSocketAddress contact) {
		...
		if (contact != null && !contact.equals(localAddress)) {
			InetSocketAddress successor = Helper.requestAddress(contact, new Request("FINDSUCC", localId, Request.DataType.ID));
			if (successor == null)  {
	...
	public String notify(InetSocketAddress successor) {
		if (successor!=null && !successor.equals(localAddress))
			return Helper.sendRequest(successor, new Request("IAMPRE", localAddress.getAddress().toString()+":"+localAddress.getPort() , Request.DataType.Address));
		else
			return null;
	}
	...
	public InetSocketAddress find_successor (long id) {
		...
		if (!pre.equals(localAddress))
			ret = Helper.requestAddress(pre, new Request("YOURSUCC", null, Request.DataType.None));
	...
	private InetSocketAddress find_predecessor (long findid) {
		...
		while (!(findid_relative_id > 0 && findid_relative_id <= n_successor_relative_id)) {
			...
			// else current node is remote node, sent request to it for its closest
				...
				InetSocketAddress result = Helper.requestAddress(n, new Request("CLOSEST", findid, Request.DataType.ID));

				// if fail to get response, set n to most recently
				if (result == null) {
					n = most_recently_alive;
					n_successor = Helper.requestAddress(n, new Request("YOURSUCC", null, Request.DataType.None));
					if (n_successor==null) {
				...
				// else n's closest is other node "result"
					...
					n_successor = Helper.requestAddress(result, new Request("YOURSUCC", null, Request.DataType.None));
	...
	public InetSocketAddress closest_preceding_finger (long findid) {
		...
		for (int i = 32; i > 0; i--) {
			...
			if (ith_finger_relative_id > 0 && ith_finger_relative_id < findid_relative)  {
				String response  = Helper.sendRequest(ith_finger, new Request("KEEP", null, Request.DataType.None));
	...
	private void deleteSuccessor() {
		...
		if ((successor == null || successor.equals(successor)) && predecessor!=null && !predecessor.equals(localAddress)) {
			...
			while (true) {
				p_pre = Helper.requestAddress(p, new Request("YOURPRE", null, Request.DataType.None));
				if (p_pre == null)
					break;
	...
	public String fetch(FilterOptions filter){
		List<TrajPoint> list = new ArrayList<>();
		StringBuilder res = new StringBuilder();
		try {
			kvStore.scan(filter, list::add);
		} catch (TrajStoreException e) {
			e.printStackTrace();
			return "";
		}
		if (list.isEmpty()) {
			LOG.info("There is no trajectory {}.", filter.getTrajectoryId());
			return "";
		}
