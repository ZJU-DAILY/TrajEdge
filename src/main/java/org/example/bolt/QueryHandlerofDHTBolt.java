 ...
 */
public class QueryHandlerofDHTBolt extends BaseBasicBolt {

    private static final Logger LOG = LoggerFactory.getLogger(QueryHandlerofDHTBolt.class);
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        Log.info("Node "+result.getAddress().toString()+", port "+result.getPort()+", position "+Helper.hexIdAndPosition(result ));


        InetSocketAddress dataHostAddress = Helper.createSocketAddress(result.getAddress().toString().substring(1)+":"+result.getPort());
        String response = Helper.sendRequest(dataHostAddress, new Request("FETCH",filter , Request.DataType.Filter));
        if(response != null){
            Log.info(trajId + " " + response);
        }
