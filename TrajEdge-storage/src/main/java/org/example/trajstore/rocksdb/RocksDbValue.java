...
class RocksDbValue {
    ...
    private static int METRIC_VALUE_SIZE = 32;
    ...
    RocksDbValue(TrajPoint p) {
        ...
        bb.putDouble(p.getOriLat());
        bb.putDouble(p.getOriLng());
    }
    ...
    double getOriLat() {
        return ByteBuffer.wrap(value, 16, 8).getDouble();
    }    
    
    double getOriLng() {
        return ByteBuffer.wrap(value, 24, 8).getDouble();
    }
